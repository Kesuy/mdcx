...
| 输入验证全面覆盖 | 部分完成 | 文件大小、命名长度、间歇数量和 CA 路径提供 validator 与行内错误；时间格式等旧保存路径仍需迁移。 |
...
| 配置声明式绑定 | 部分完成 | schema 已覆盖媒体路径、清理、NFO、翻译/LLM、命名、字幕、Emby、Cookie、日志、网络等 90 余个控件；枚举组合和时间字段仍由专用逻辑处理。 |
...
3. 将时间、枚举组合等剩余配置字段纳入声明式 schema，并补齐格式校验。
