...
| 配置声明式绑定 | 部分完成 | schema 已覆盖 90 余个普通控件、常用单选枚举，以及过滤、清理、高清图来源、读取模式、下载与保留文件等多选枚举；字段语言/翻译和有派生值的复合集合仍由专用逻辑处理。 |

## 后续顺序

1. 继续按页面拆分生成 UI，优先剩余设置页，逐步删除 `responsive_layout.py` 的兼容 geometry 路径。
2. 将字段语言/翻译和有派生值的复合集合继续收敛到可测试的专用 schema；字段网站优先级保留在有序拖放编辑器中。
