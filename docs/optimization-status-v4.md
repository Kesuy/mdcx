...
| 输入验证全面覆盖 | 部分完成 | 文件大小、命名长度、间歇数量、`HH:MM:SS` 时间、服务 URL 和 CA 路径提供 validator 与行内错误；模板字段仍需补充。 |
...
| 主窗口职责拆分 | 部分完成 | 结果、设置、任务、网络、刮削、NFO、删除和链接创建已拆出；部分工具动作仍位于主窗口。 |
...
2. 将剩余文件/工具动作迁入 `FileController` 或独立工具控制器，继续缩小 `MyMAinWindow`。
3. 将枚举组合等剩余配置字段纳入声明式 schema，并补齐命名模板格式校验。
