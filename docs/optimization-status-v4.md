...
| 输入验证全面覆盖 | 部分完成 | 文件大小、命名长度、间歇数量、`HH:MM:SS` 时间和 CA 路径提供 validator 与行内错误；少量网址及模板字段仍需补充。 |
...
| 主窗口职责拆分 | 部分完成 | 结果、设置、任务、网络、刮削、NFO 和删除操作已拆出；链接创建及部分工具动作仍位于主窗口。 |
| 配置声明式绑定 | 部分完成 | schema 已覆盖媒体路径、清理、NFO、翻译/LLM、命名、字幕、Emby、Cookie、日志、网络和时间字段等 90 余个控件；枚举组合仍由专用逻辑处理。 |

## 后续顺序

1. 继续按页面拆分生成 UI，优先剩余设置页，逐步删除 `responsive_layout.py` 的兼容 geometry 路径。
2. 将链接创建和剩余文件/工具动作迁入 `FileController` 或独立工具控制器，继续缩小 `MyMAinWindow`。
3. 将枚举组合等剩余配置字段纳入声明式 schema，并补齐网址和模板格式校验。
4. 把生成 UI 和旧页面中剩余硬编码 QSS 收敛到主题 token。
